package repositories

import (
	"bytes"
	"context"
	"encoding/csv"
	"fmt"
	"os"
	"strconv"
	"strings"

	"github.com/shopspring/decimal"

	"github.com/robmux/transaction-summary-notifier/pkg/domains/transactions"
)

type CsvExtractor struct {
}

func New() *CsvExtractor {
	return &CsvExtractor{}
}

func (c *CsvExtractor) GetUserTransactions(
	ctx context.Context,
	userID uint64) ([]transactions.TransactionDetail, error) {
	wd, err := os.Getwd()
	if err != nil {
		return nil, err
	}

	path := fmt.Sprintf("%s/input/user_%d_transactions.csv", wd, userID)

	csvFile, err := loadFile(path)
	if err != nil {
		return nil, err
	}

	columns := []string{
		"TxID", "Date", "TransactionAmount",
	}

	transactionDetails, err := readCSV(columns, csvFile)
	if err != nil {
		return nil, err
	}

	return transactionDetails, nil
}

// loadFile loads the file completely by putting it into memory.
func loadFile(filePath string) ([]byte, error) {
	data, err := os.ReadFile(filePath)
	if err != nil {
		return nil, err
	}

	return data, nil
}

func readCSV(columns []string, csvBytes []byte) ([]transactions.TransactionDetail, error) {
	csvReader := csv.NewReader(bytes.NewReader(csvBytes))
	data, err := csvReader.ReadAll()
	if err != nil {
		return nil, err
	}

	transactionDetails := make([]transactions.TransactionDetail, 0, len(data))
	for i, line := range data {
		if i == 0 {
			// TODO: validate headers are the expected
			continue
		}

		if len(line) != len(columns) {
			return nil, fmt.Errorf("line %d has %d columns but should have exactly %d", i, len(line), len(columns))
		}

		transactionX := transactions.TransactionDetail{}
		for j, column := range line {
			switch columns[j] {
			case "TxID":
				transactionX.ID, err = strconv.ParseUint(column, 10, 64)
				if err != nil {
					return nil, err
				}
			case "Date":
				dayMonthStrs := strings.Split(column, "/")
				if len(dayMonthStrs) != 2 {
					return nil, fmt.Errorf("invalid date values %s", column)
				}
				month, err := strconv.ParseUint(dayMonthStrs[0], 10, 64)
				if err != nil {
					return nil, err
				}
				day, err := strconv.ParseUint(dayMonthStrs[1], 10, 64)
				if err != nil {
					return nil, err
				}

				if day > 31 {
					return nil, fmt.Errorf("invalid day number %d", day)
				}
				if month > 12 {
					return nil, fmt.Errorf("invalid month number %d", month)
				}

				transactionX.Date = transactions.MonthDay{
					Day:   uint8(day),
					Month: uint8(month),
				}

			case "TransactionAmount":
				if len(column) <= 1 {
					return nil, fmt.Errorf("invalid transaction amount %s", column)
				}

				if column[0] != '-' && column[0] != '+' {
					return nil, fmt.Errorf("invalid transaction amount, does not have sign %s", column)
				}

				amountFloat, err := strconv.ParseFloat(column, 64)
				if err != nil {
					return nil, err
				}
				transactionX.TransactionAmount = decimal.NewFromFloat(amountFloat)
			}
		}

		transactionDetails = append(transactionDetails, transactionX)
	}

	return transactionDetails, nil
}
